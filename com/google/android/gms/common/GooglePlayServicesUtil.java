...
public final class GooglePlayServicesUtil extends zze {
    ...
    private static void zza(int i, Context context, String str) {
        zza(i, context, str, GoogleApiAvailability.getInstance().zza(context, i, GOOGLE_PLAY_SERVICES_VERSION_CODE, "n"));
    }
    ...
    private static void zza(int i, Context context, String str, PendingIntent pendingIntent) {
        ...
        if (zzg == null) {
            zzg = resources.getString(C0219R.string.common_google_play_services_notification_ticker);
        }
        CharSequence zzd = zzh.zzd(context, i, zzbv);
        if (com.google.android.gms.common.util.zzi.zzck(context)) {
            zzab.zzbm(zzs.zzavn());
            build = new Notification.Builder(context).setSmallIcon(C0219R.drawable.common_ic_googleplayservices).setPriority(2).setAutoCancel(true).setStyle(new BigTextStyle().bigText(new StringBuilder((String.valueOf(zzg).length() + 1) + String.valueOf(zzd).length()).append(zzg).append(" ").append(zzd).toString())).addAction(C0219R.drawable.common_full_open_on_phone, resources.getString(C0219R.string.common_open_on_phone), pendingIntent).build();
        } else {
            CharSequence string = resources.getString(C0219R.string.common_google_play_services_notification_ticker);
            if (zzs.zzavj()) {
                Notification build2;
