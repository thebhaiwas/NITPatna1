...
public final class zzanr {
    ...
    private static final class zzb implements Serializable, ParameterizedType {
        ...
        public String toString() {
            StringBuilder stringBuilder = new StringBuilder((this.beK.length + 1) * 30);
            stringBuilder.append(zzanr.zzg(this.beJ));
            if (this.beK.length == 0) {
                return stringBuilder.toString();
            }
            stringBuilder.append("<").append(zzanr.zzg(this.beK[0]));
            for (int i = 1; i < this.beK.length; i++) {
                stringBuilder.append(", ").append(zzanr.zzg(this.beK[i]));
            }
    ...
    public static Type zze(Type type) {
        if (type instanceof Class) {
            zza com_google_android_gms_internal_zzanr_zza;
            Class cls = (Class) type;
            if (cls.isArray()) {
                com_google_android_gms_internal_zzanr_zza = new zza(zze(cls.getComponentType()));
            } else {
                Object obj = cls;
            }
            return com_google_android_gms_internal_zzanr_zza;
