...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable {
    ...
    class C03271 extends OutputStream {
        C03271() {
        }
    ...
    class C03282 extends InputStream {
        C03282() {
        }
    ...
    public long completeSegmentByteCount() {
        ...
        if (tail.limit < ItemAnimator.FLAG_MOVED && tail.owner) {
    ...
    public int readUtf8CodePoint() throws EOFException {
        ...
        int min;
        byte b0 = getByte(0);
        if ((b0 & TransportMediator.FLAG_KEY_MEDIA_NEXT) == 0) {
            ...
            codePoint = b0 & 31;
            byteCount = 2;
            min = TransportMediator.FLAG_KEY_MEDIA_NEXT;
        } else if ((b0 & 240) == 224) {
            codePoint = b0 & 15;
            byteCount = 3;
            min = ItemAnimator.FLAG_MOVED;
        } else if ((b0 & 248) == 240) {
            codePoint = b0 & 7;
            byteCount = 4;
            min = AccessibilityNodeInfoCompat.ACTION_CUT;
        } else {
            skip(1);
    ...
    public Buffer writeUtf8(String string, int beginIndex, int endIndex) {
        if (string == null) {
            ...
            while (i < endIndex) {
                ...
                if (c < TransportMediator.FLAG_KEY_MEDIA_NEXT) {
                    Segment tail = writableSegment(1);
                    byte[] data = tail.data;
                    ...
                    writeByte((c >> 6) | 192);
                    writeByte((c & 63) | TransportMediator.FLAG_KEY_MEDIA_NEXT);
                    i2 = i + 1;
                } else if (c < 55296 || c > 57343) {
                    writeByte((c >> 12) | 224);
                    writeByte(((c >> 6) & 63) | TransportMediator.FLAG_KEY_MEDIA_NEXT);
                    writeByte((c & 63) | TransportMediator.FLAG_KEY_MEDIA_NEXT);
                    i2 = i + 1;
                } else {
                    int low;
                    ...
                    if (c > 56319 || low < 56320 || low > 57343) {
                        ...
                        writeByte(((codePoint >> 12) & 63) | TransportMediator.FLAG_KEY_MEDIA_NEXT);
                        writeByte(((codePoint >> 6) & 63) | TransportMediator.FLAG_KEY_MEDIA_NEXT);
                        writeByte((codePoint & 63) | TransportMediator.FLAG_KEY_MEDIA_NEXT);
                        i2 = i + 2;
                    }
                }
                i = i2;
    ...
    public Buffer writeUtf8CodePoint(int codePoint) {
        if (codePoint < TransportMediator.FLAG_KEY_MEDIA_NEXT) {
            writeByte(codePoint);
        } else if (codePoint < ItemAnimator.FLAG_MOVED) {
            writeByte((codePoint >> 6) | 192);
            writeByte((codePoint & 63) | TransportMediator.FLAG_KEY_MEDIA_NEXT);
        } else if (codePoint < AccessibilityNodeInfoCompat.ACTION_CUT) {
            if (codePoint < 55296 || codePoint > 57343) {
    ...
    Segment writableSegment(int minimumCapacity) {
        if (minimumCapacity < 1 || minimumCapacity > ItemAnimator.FLAG_MOVED) {
            ...
            r0 = this.head.prev;
            if (r0.limit + minimumCapacity > ItemAnimator.FLAG_MOVED || !r0.owner) {
