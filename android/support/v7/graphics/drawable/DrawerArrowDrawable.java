...
public class DrawerArrowDrawable extends Drawable {
    ...
    public DrawerArrowDrawable(Context context) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(null, C0158R.styleable.DrawerArrowToggle, C0158R.attr.drawerArrowStyle, C0158R.style.Base_Widget_AppCompat_DrawerArrowToggle);
        setColor(a.getColor(C0158R.styleable.DrawerArrowToggle_color, ARROW_DIRECTION_LEFT));
        setBarThickness(a.getDimension(C0158R.styleable.DrawerArrowToggle_thickness, 0.0f));
        setSpinEnabled(a.getBoolean(C0158R.styleable.DrawerArrowToggle_spinBars, true));
        setGapSize((float) Math.round(a.getDimension(C0158R.styleable.DrawerArrowToggle_gapBetweenBars, 0.0f)));
        this.mSize = a.getDimensionPixelSize(C0158R.styleable.DrawerArrowToggle_drawableSize, ARROW_DIRECTION_LEFT);
        this.mBarLength = (float) Math.round(a.getDimension(C0158R.styleable.DrawerArrowToggle_barLength, 0.0f));
        this.mArrowHeadLength = (float) Math.round(a.getDimension(C0158R.styleable.DrawerArrowToggle_arrowHeadLength, 0.0f));
        this.mArrowShaftLength = a.getDimension(C0158R.styleable.DrawerArrowToggle_arrowShaftLength, 0.0f);
        a.recycle();
    }
    ...
    public void setBarThickness(float width) {
        if (this.mPaint.getStrokeWidth() != width) {
            ...
            this.mMaxCutForBarSize = (float) (((double) (width / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT)) * Math.cos((double) ARROW_HEAD_ANGLE));
            invalidateSelf();
        }
    }
    ...
    public void draw(Canvas canvas) {
        ...
        float arrowHeadBarLength = lerp(this.mBarLength, (float) Math.sqrt((double) ((this.mArrowHeadLength * this.mArrowHeadLength) * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT)), this.mProgress);
        ...
        float arrowEdge = (-arrowShaftLength) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT;
        this.mPath.moveTo(arrowEdge + arrowShaftCut, 0.0f);
        this.mPath.rLineTo(arrowShaftLength - (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * arrowShaftCut), 0.0f);
        ...
        canvas.translate((float) bounds.centerX(), (float) (((double) ((float) ((((int) ((((float) bounds.height()) - (3.0f * barThickness)) - (this.mBarGap * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT))) / 4) * ARROW_DIRECTION_START))) + ((((double) barThickness) * 1.5d) + ((double) this.mBarGap))));
        if (this.mSpin) {
