...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    void layoutChunk(Recycler recycler, State state, LayoutState layoutState, LayoutChunkResult result) {
        ...
        for (i = 0; i < count; i++) {
            ...
            int spec = LayoutManager.getChildMeasureSpec(this.mCachedBorders[lp.mSpanIndex + lp.mSpanSize] - this.mCachedBorders[lp.mSpanIndex], otherDirSpecMode, 0, this.mOrientation == 0 ? lp.height : lp.width, DEBUG);
            ...
            if (this.mOrientation == 1) {
                measureChildWithDecorationsAndMargin(view, spec, mainSpec, lp.height == DEFAULT_SPAN_COUNT ? true : DEBUG, DEBUG);
            } else {
                measureChildWithDecorationsAndMargin(view, mainSpec, spec, lp.width == DEFAULT_SPAN_COUNT ? true : DEBUG, DEBUG);
            }
        ...
        if (flexibleInOtherDir) {
            ...
            for (i = 0; i < count; i++) {
                ...
                spec = LayoutManager.getChildMeasureSpec(this.mCachedBorders[lp.mSpanIndex + lp.mSpanSize] - this.mCachedBorders[lp.mSpanIndex], 1073741824, 0, this.mOrientation == 0 ? lp.height : lp.width, DEBUG);
                ...
                if (this.mOrientation == 1) {
                    measureChildWithDecorationsAndMargin(view, spec, mainSpec, DEBUG, true);
                } else {
                    measureChildWithDecorationsAndMargin(view, mainSpec, spec, DEBUG, true);
                }
