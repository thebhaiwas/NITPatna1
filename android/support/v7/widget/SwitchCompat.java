...
public class SwitchCompat extends CompoundButton {
    ...
    class C02031 implements AnimationListener {
        ...
        public void onAnimationEnd(Animation animation) {
            if (SwitchCompat.this.mPositionAnimator == animation) {
                SwitchCompat.this.setThumbPosition(this.val$newCheckedState ? DefaultRetryPolicy.DEFAULT_BACKOFF_MULT : 0.0f);
    ...
    public SwitchCompat(Context context, AttributeSet attrs) {
        this(context, attrs, C0158R.attr.switchStyle);
    }
    ...
    public SwitchCompat(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, C0158R.styleable.SwitchCompat, defStyleAttr, 0);
        this.mThumbDrawable = a.getDrawable(C0158R.styleable.SwitchCompat_android_thumb);
        if (this.mThumbDrawable != null) {
            this.mThumbDrawable.setCallback(this);
        }
        this.mTrackDrawable = a.getDrawable(C0158R.styleable.SwitchCompat_track);
        if (this.mTrackDrawable != null) {
            this.mTrackDrawable.setCallback(this);
        }
        this.mTextOn = a.getText(C0158R.styleable.SwitchCompat_android_textOn);
        this.mTextOff = a.getText(C0158R.styleable.SwitchCompat_android_textOff);
        this.mShowText = a.getBoolean(C0158R.styleable.SwitchCompat_showText, true);
        this.mThumbTextPadding = a.getDimensionPixelSize(C0158R.styleable.SwitchCompat_thumbTextPadding, 0);
        this.mSwitchMinWidth = a.getDimensionPixelSize(C0158R.styleable.SwitchCompat_switchMinWidth, 0);
        this.mSwitchPadding = a.getDimensionPixelSize(C0158R.styleable.SwitchCompat_switchPadding, 0);
        this.mSplitTrack = a.getBoolean(C0158R.styleable.SwitchCompat_splitTrack, false);
        int appearance = a.getResourceId(C0158R.styleable.SwitchCompat_switchTextAppearance, 0);
        if (appearance != 0) {
    ...
    public void setSwitchTextAppearance(Context context, int resid) {
        TypedArray appearance = context.obtainStyledAttributes(resid, C0158R.styleable.TextAppearance);
        ColorStateList colors = appearance.getColorStateList(C0158R.styleable.TextAppearance_android_textColor);
        if (colors != null) {
            this.mTextColors = colors;
        } else {
            this.mTextColors = getTextColors();
        }
        int ts = appearance.getDimensionPixelSize(C0158R.styleable.TextAppearance_android_textSize, 0);
        if (!(ts == 0 || ((float) ts) == this.mTextPaint.getTextSize())) {
            this.mTextPaint.setTextSize((float) ts);
            requestLayout();
        }
        setSwitchTypefaceByIndex(appearance.getInt(C0158R.styleable.TextAppearance_android_typeface, -1), appearance.getInt(C0158R.styleable.TextAppearance_android_textStyle, -1));
        if (appearance.getBoolean(C0158R.styleable.TextAppearance_textAllCaps, false)) {
    ...
    private Layout makeLayout(CharSequence text) {
        ...
        return new StaticLayout(transformed, this.mTextPaint, transformed != null ? (int) Math.ceil((double) Layout.getDesiredWidth(transformed, this.mTextPaint)) : 0, Alignment.ALIGN_NORMAL, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, 0.0f, true);
    }
            ...
            case TOUCH_MODE_DRAGGING /*2*/:
                switch (this.mTouchMode) {
                    ...
                    case TOUCH_MODE_DRAGGING /*2*/:
                        ...
                        float newPos = constrain(this.mThumbPosition + dPos, 0.0f, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
    ...
    private void animateThumbToCheckedState(boolean newCheckedState) {
        ...
        this.mPositionAnimator = new ThumbAnimation(this.mThumbPosition, newCheckedState ? DefaultRetryPolicy.DEFAULT_BACKOFF_MULT : 0.0f, null);
        this.mPositionAnimator.setDuration(250);
        this.mPositionAnimator.setAnimationListener(new C02031(newCheckedState));
    ...
    private int getThumbOffset() {
        float thumbPosition;
        if (ViewUtils.isLayoutRtl(this)) {
            thumbPosition = DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - this.mThumbPosition;
        } else {
    ...
    public void jumpDrawablesToCurrentState() {
        if (VERSION.SDK_INT >= 11) {
            ...
            cancelPositionAnimator();
            setThumbPosition(isChecked() ? DefaultRetryPolicy.DEFAULT_BACKOFF_MULT : 0.0f);
        }
    }
