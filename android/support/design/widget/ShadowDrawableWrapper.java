...
class ShadowDrawableWrapper extends DrawableWrapper {
    ...
    private void drawShadow(Canvas canvas) {
        ...
        boolean drawHorizontalEdges = this.mContentBounds.width() - (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * shadowOffset) > 0.0f;
        boolean drawVerticalEdges = this.mContentBounds.height() - (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * shadowOffset) > 0.0f;
        ...
        if (drawHorizontalEdges) {
            ...
            canvas.drawRect(0.0f, edgeShadowTop, this.mContentBounds.width() - (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * shadowOffset), -this.mCornerRadius, this.mEdgeShadowPaint);
        }
        ...
        if (drawHorizontalEdges) {
            canvas.scale(SHADOW_BOTTOM_SCALE / shadowScaleHorizontal, SHADOW_BOTTOM_SCALE);
            canvas.drawRect(0.0f, edgeShadowTop, this.mContentBounds.width() - (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * shadowOffset), this.mShadowSize + (-this.mCornerRadius), this.mEdgeShadowPaint);
        }
        canvas.restoreToCount(saved);
        saved = canvas.save();
        ...
        if (drawVerticalEdges) {
            ...
            canvas.drawRect(0.0f, edgeShadowTop, this.mContentBounds.height() - (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * shadowOffset), -this.mCornerRadius, this.mEdgeShadowPaint);
        }
        canvas.restoreToCount(saved);
        saved = canvas.save();
    ...
    private void buildShadowCorners() {
        ...
        if (shadowRadius > 0.0f) {
            ...
            float midRatio = startRatio + ((SHADOW_BOTTOM_SCALE - startRatio) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT);
    ...
    public float getMinWidth() {
        return (this.mRawMaxShadowSize * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) + (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * Math.max(this.mRawMaxShadowSize, this.mCornerRadius + (this.mRawMaxShadowSize / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT)));
    }

    public float getMinHeight() {
        return ((this.mRawMaxShadowSize * SHADOW_MULTIPLIER) * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) + (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * Math.max(this.mRawMaxShadowSize, this.mCornerRadius + ((this.mRawMaxShadowSize * SHADOW_MULTIPLIER) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT)));
    }
}
