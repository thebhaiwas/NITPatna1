...
public class TextInputLayout extends LinearLayout {
    ...
    class C03634 implements AnimatorUpdateListener {
        C03634() {
        }
    ...
    class C05582 extends ViewPropertyAnimatorListenerAdapter {
        C05582() {
        }
    ...
    class C05593 extends ViewPropertyAnimatorListenerAdapter {
        final /* synthetic */ CharSequence val$error;

        C05593(CharSequence charSequence) {
    ...
    public void setError(@Nullable CharSequence error) {
        if (!TextUtils.equals(this.mError, error)) {
            ...
            if (this.mErrorShown) {
                ...
                if (animate) {
                    if (ViewCompat.getAlpha(this.mErrorView) == DefaultRetryPolicy.DEFAULT_BACKOFF_MULT) {
                        ViewCompat.setAlpha(this.mErrorView, 0.0f);
                    }
                    ViewCompat.animate(this.mErrorView).alpha(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT).setDuration(200).setInterpolator(AnimationUtils.LINEAR_OUT_SLOW_IN_INTERPOLATOR).setListener(new C05582()).start();
                }
            } else if (this.mErrorView.getVisibility() == 0) {
                if (animate) {
                    ViewCompat.animate(this.mErrorView).alpha(0.0f).setDuration(200).setInterpolator(AnimationUtils.FAST_OUT_LINEAR_IN_INTERPOLATOR).setListener(new C05593(error)).start();
                } else {
                    this.mErrorView.setVisibility(4);
                }
            }
    ...
    private void animateToExpansionFraction(float target) {
        if (this.mCollapsingTextHelper.getExpansionFraction() != target) {
            if (this.mAnimator == null) {
                ...
                this.mAnimator.setUpdateListener(new C03634());
            }
