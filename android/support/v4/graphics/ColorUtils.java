...
public final class ColorUtils {
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int r, @IntRange(from = 0, to = 255) int g, @IntRange(from = 0, to = 255) int b, @NonNull float[] outHsl) {
        ...
        float l = (max + min) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT;
        if (max == min) {
            s = 0.0f;
            h = 0.0f;
        } else {
            if (max == rf) {
                ...
                h = ((bf - rf) / deltaMaxMin) + ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT;
            } else {
                h = ((rf - gf) / deltaMaxMin) + 4.0f;
            }
            s = deltaMaxMin / (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - Math.abs((ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * l) - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
        }
        h = (60.0f * h) % 360.0f;
        if (h < 0.0f) {
            h += 360.0f;
        ...
        outHsl[MIN_ALPHA_SEARCH_PRECISION] = constrain(s, 0.0f, (float) DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
    ...
    public static int HSLToColor(@NonNull float[] hsl) {
        ...
        float l = hsl[2];
        float c = (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - Math.abs((ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * l) - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT)) * s;
        float m = l - (0.5f * c);
        float x = c * (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - Math.abs(((h / 60.0f) % ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
        int r = 0;
        int g = 0;
        int b = 0;
        switch (((int) h) / 60) {
    ...
    public static int XYZToColor(@FloatRange(from = 0.0d, to = 95.047d) double x, @FloatRange(from = 0.0d, to = 100.0d) double y, @FloatRange(from = 0.0d, to = 108.883d) double z) {
        ...
        return Color.rgb(constrain((int) Math.round(255.0d * (r > 0.0031308d ? (1.055d * Math.pow(r, 0.4166666666666667d)) - 0.055d : r * 12.92d)), 0, (int) MotionEventCompat.ACTION_MASK), constrain((int) Math.round(255.0d * (g > 0.0031308d ? (1.055d * Math.pow(g, 0.4166666666666667d)) - 0.055d : g * 12.92d)), 0, (int) MotionEventCompat.ACTION_MASK), constrain((int) Math.round(255.0d * (b > 0.0031308d ? (1.055d * Math.pow(b, 0.4166666666666667d)) - 0.055d : b * 12.92d)), 0, (int) MotionEventCompat.ACTION_MASK));
    }
